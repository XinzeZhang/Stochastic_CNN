...
lastTrain_epochs = 200 - microepochs

GD_epochs = 100 # epoch times which lr stoped reduced to 0.0001 
batch_size =128
iterations_of_epoch = int(math.ceil(train_size/batch_size))
data_dir = '/tmp/cifar10_data/cifar-10-batches-bin'
result_dir1 = "./Result_1L"
if os.path.exists(result_dir1) == False:
    os.mkdir(result_dir1)
result_dir2 = "./Result_1L/"+str(kernel_num)
if os.path.exists(result_dir2) == False:
...
#训练开始
# -----------------------------------------------------------------------
# gradually decrease the lr to lr/10
lr = 0.001
end_lr = lr/10
window =5
if microepochs==1:
    reduced_times=150/window
    times=300/window
else:
    reduced_times=(150-microepochs)/window
    times=(300-microepochs)/window


lr_reduced = (lr-end_lr)/reduced_times
...
for i_epoch in range(microepochs):
    ...
    my_global_epoch = my_global_epoch+1


    for iterations in range(iterations_of_epoch):
        image_batch,label_batch = sess.run([images_train,labels_train])
        sess.run([train_op],feed_dict={image_holder:image_batch,label_holder:label_batch,learning_rate:lr})
...
for t in range(int(times)):
    ...
    for epoch in range(window):
        ...
        loss_count = 0
        my_global_epoch = my_global_epoch + 1
        # if my_global_epoch % window == 0 and lr>=end_lr:
        #     lr = lr-lr_reduced
        #     lr = round(lr,5)
        print(my_global_epoch,lr)
        total_sample_count = iterations_of_epoch *batch_size
        for iterations in range(iterations_of_epoch):
            image_batch,label_batch = sess2.run([images_train,labels_train])
            _ = sess2.run([train_op2],feed_dict={image_holder: image_batch, label_holder: label_batch,learning_rate:lr})
        ...
        print('epoch =%d test_acc =%.5f' % (my_global_epoch, test_acc))
        with open("./Result_1L/outputlog.txt", "a+") as f:
            # print('epoch =%d test_loss =%.5f train_loss = %.5f' % (i_epoch, test_loss, train_loss), file=f)
            print('epoch =%d test_acc =%.5f train_acc = %.5f' % (my_global_epoch, test_acc, train_acc), file=f)        


# for i_epoch in range(lastTrain_epochs):
#     true_count = 0
#     loss_count = 0
#     my_global_epoch = my_global_epoch + 1
#     if my_global_epoch % window == 0 and lr>=end_lr:
