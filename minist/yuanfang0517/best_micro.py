...
if os.path.exists("./Result_npz_BN_best") == False:
    os.mkdir("./Result_npz_BN_best")
...
train_acc_array = []
best_acc = 0
global temp_biases,temp_weights
# Start training
with tf.Session() as sess:
    ...
    for step in range(1, microtrain_steps+1):
        ...
        if step % display_step == 0 :
            ...
            print("Epoch " + str(int(step/display_step)) + ", Minibatch Loss= " + \
                  "{:.4f}".format(test_loss) + ", test Accuracy= " + \
                  "{:.5f}".format(test_acc))
    print("cost_time: %.6f"% (time.time() - start))
    print("Optimization Finished!  best test acc = ",best_acc)
...
optimizer2 = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(cost2)
...
with tf.Session() as sess:
    ...
    np.savez("./Result_npz_BN_best/acc"+str(int(microtrain_steps/display_step))+".npz", test_acc_array, train_acc_array)
    np.savez("./Result_npz_BN_best/loss"+str(int(microtrain_steps/display_step))+".npz", test_loss_array, train_loss_array)
